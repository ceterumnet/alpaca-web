...
describe('SettingsPanelMigrated.vue', () => {
  ...
  beforeEach(() => {
    ...
    // Set up the mocks
    vi.mocked(useUIPreferencesStore).mockReturnValue(uiStore as unknown as ReturnType<typeof useUIPreferencesStore>)
    vi.mocked(useLayoutStore).mockReturnValue(layoutStore as unknown as ReturnType<typeof useLayoutStore>)
    vi.mocked(useNotificationStore).mockReturnValue(notificationStore as unknown as ReturnType<typeof useNotificationStore>)
  ...
  it('renders correctly with tabs', () => {
    ...
    expect(wrapper.findAll('.aw-settings__tab-btn').length).toBe(5)

    // First tab (appearance) should be active by default
    const activeTab = wrapper.find('.aw-settings__tab-btn--active')
  ...
  it('switches tabs when clicked', async () => {
    ...
    expect(wrapper.find('[data-testid="tab-appearance"]').classes()).toContain('aw-settings__tab-btn--active')

    // Click on the Layout tab
    await wrapper.find('[data-testid="tab-layout"]').trigger('click')

    // Layout tab should now be active
    expect(wrapper.find('[data-testid="tab-layout"]').classes()).toContain('aw-settings__tab-btn--active')
  ...
  it('resets UI preferences when reset button is clicked', async () => {
    ...
    // Call resetSettings directly instead of clicking the button
    await vm.resetSettings()

    // Verify notification
    expect(notificationStore.showInfo).toHaveBeenCalledWith('Settings reset to current values.')
  })
  ...
  it('handles invalid JSON during import', async () => {
    ...
    expect(notificationStore.showError).toHaveBeenCalledWith(expect.stringContaining('Failed to import settings'))
  })
})
