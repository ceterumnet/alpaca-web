...
describe('DiscoveredDevicesMigrated.vue', () => {
  ...
  it('emits device-connected event when connecting to a device', async () => {
    const wrapper = mount(DiscoveredDevices)
    await flushPromises()
    ...
    // Verify that the device-connected event was emitted
    expect(emitSpy).toHaveBeenCalledWith('device-connected', 'Test Device')
  })

  it('emits connection-error event when connection fails', async () => {
    const wrapper = mount(DiscoveredDevices)
    await flushPromises()
    ...
    // Mock the connectDevice to throw an error
    ...
    // Verify that the connection-error event was emitted
    expect(emitSpy).toHaveBeenCalledWith('connection-error', 'Test Device', 'Connection failed')
  })
})
