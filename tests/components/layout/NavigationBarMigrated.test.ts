...
import NavigationBarMigrated from '../../../src/components/layout/NavigationBarMigrated.vue'
...
describe('NavigationBarMigrated.vue', () => {
  ...
  it('hides device status when no devices exist', async () => {
    // Setup the store with no devices
    const unifiedStore = useUnifiedStore()
    Object.defineProperty(unifiedStore, 'devicesList', {
      value: [],
      writable: true
    ...
    // Should not show device status indicator when there are no devices
