...
export class DirectDiscoveryService implements IDeviceDiscoveryService {
  ...
  async discoverDevices(options?: DiscoveryOptions): Promise<DiscoveryResult> {
    ...
    debugLog('Starting direct device discovery with options:', options)
    ...
    try {
      const response = await axios.get<{ value: ConfiguredAlpacaDevice[] }>(`${baseUrl}/management/v1/configureddevices`)
