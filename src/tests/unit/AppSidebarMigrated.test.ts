...
describe('AppSidebarMigrated.vue', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('renders device items with correct information', () => {
    ...
    // Check icon type
    expect(deviceItems[0].find('.icon').attributes('data-icon-type')).toBe('search')
    ...
    expect(deviceItems[1].find('.icon').attributes('data-icon-type')).toBe('camera')
  })
